...
object Utils {
  ...
  def getInputFromUser(): String = {
    ...
    val fileExists = !readFromFile(s"src/$userInput.txt").isEmpty
    fileExists match {
      case true => userInput
      case _ =>
        getInputFromUser()
  ...
  def getSizeInput(): Double = {
    println("Please type a size for the desired octree.")
    val input = scala.io.StdIn.readDouble()
    if (input > 32.0) {
      println("Size limit is 32")
      getSizeInput()
    } else
      input
  }
  ...
  def readFromFile(file: String): List[Node] = {
    if (!Files.exists(Paths.get(file)) || Source.fromFile(file) == null) {
